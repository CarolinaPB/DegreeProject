...
hotspot_name <- unique(vars_select(names(test), starts_with("chrXII:")))
...
```{r}
genes_in_hotspot <- genes_inside_hotspot(hotspot_data = hotspot_data.interval,coordinates_plot_links.200, 12, lefttlim = 5, rightlim = 6)
```
...
```{r}
genes_in_hotspot <- genes_inside_hotspot(hotspot_data = hotspot_data.interval,coordinates_plot_links.200, 14, lefttlim = 5, rightlim = 6)
```
...
```{r}
genes_in_hotspot <- genes_inside_hotspot(hotspot_data = hotspot_data.interval,coordinates_plot_links, c(12, 13), lefttlim = 5, rightlim = 6)
```

```{r}
coordinates_plot_links.200.A <- unique(coordinates_plot_links.200[,.(geneA, start.A, chr.A,count.A )])
hotspot_data.interval[chromosome %in% chrom_to_plot]
```
