...
#One might also want to consider reduction of the universe to exclude genes that are not expressed, if such a determination can be made, since arguments can be made against maintaining objects in the universe that cannot be selected. The

# distance between GO terms in termgrA - graph 1
# GO_genesA_dist <- compGdist(termgrA[[1]], nodes(termgrA[[1]]), verbose = T)
# GO_genesA_dist.dt <- data.table(t, keep.rownames=T)
# 
# 
# 
# prof <- basicProfile(genelist=genesA, idType = "GoTermsFrame", onto="BP", level=2, empty.cats=T)
# 
