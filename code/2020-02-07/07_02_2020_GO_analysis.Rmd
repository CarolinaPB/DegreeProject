...
### table with gene, GO and cluster
```{r}
ympy.GO.cluster <- unique(merge(ympy.GO, cluster_table, by.x="gene", by.y="node", all=T, allow.cartesian=TRUE))
ympy.GO.cluster
```

# find genes that have "transcription factor" in the GO term
```{r}
GO_trans_factor <- unique(ympy.GO.cluster[grepl("transcription factor",ontologyTerm.name, fixed = F)])
# with cluster
GO_trans_factor
# without cluster
...
There are `r length(unique(GO_trans_factor[,.(gene, ontologyTerm.identifier, ontologyTerm.name)]$gene))` different genes that have "Transcription factor" in its GO term  
...
```{r}
links_perGO_pergene <- merge(unique(ympy.GO), numlinks_from_gene, by.x="gene", by.y="geneA", all=T)
...
### Number of times certain GO terms appear (in causal links)
```{r}
links_perGO <- links_perGO_pergene[,.(gocount=sum(count, na.rm = T)), by=c("ontologyTerm.name", "ontologyTerm.identifier")]
links_perGO[,.(ontologyTerm.name, gocount, ontologyTerm.identifier)][order(-gocount)]
```
