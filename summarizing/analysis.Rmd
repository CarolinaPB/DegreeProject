...
```
ERC1 is affecting `r length(unique(find.effects_TF.names[symbol=="ERC1" & grepl("methionine",GO.term.B, fixed = F)]$geneB))` genes that have a GO term associated with the methionine pathway. The genes are `r unique(find.effects_TF.names[symbol=="ERC1" & grepl("methionine",GO.term.B, fixed = F)]$geneB)` (`r unique(find.effects_TF.names[symbol=="ERC1" & grepl("methionine",GO.term.B, fixed = F)]$symbol.B)`)  
...
```{r}
plot(x=chi2.res$h.cutoff, y=chi2.res$`O/U.causal.nothot`, xlab="h2 cutoff", ylab="Over/under", type="b", col="darkgreen", pch=1, cex.lab=1.5, lwd=2)
lines(x=chi2.res$h.cutoff, y=chi2.res$`O/U.causal.hot`, type="b", col="green", pch=2, lwd=2)
lines(x=chi2.res$h.cutoff, y=chi2.res$`O/U.notcausal.nothot`, type="b", col="darkblue", pch=3, lwd=2)
lines(x=chi2.res$h.cutoff, y=chi2.res$`O/U.notcausal.hot`, type="b", col="blue", pch=4, lwd=2)
legend("topright", legend=c("Causal not in hotspot", "Causal in hotspot", "Not causal not in hotspot", "Not causal in hotspot"),
       col=c("darkgreen", "green", "darkblue", "blue"), lty=1, cex=0.8, pch=1:4,lwd=2)
```
...
```
Out of `r nrow(unique(find.effects_TF.chr[,.(geneA, geneB, eqtl.A)]))` geneA affecting geneB pairs, `r nrow(unique(find.effects_TF.trans[,.(geneB, eqtl.A)]))` geneB-eqtlA pairs were in the reported list.  
There are `r nrow(find.effects_TF.trans[cis==T])` cases where the gene and eqtl are in cis and `r nrow(unique(find.effects_TF.trans[,.(geneB, eqtl.A, cis)][cis==F]))` where eqtlA is in trans with geneB
